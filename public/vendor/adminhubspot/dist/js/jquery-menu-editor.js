...
function MenuEditor(idSelector, options) {
    ...
    $(document).on('click', '.btnRemove', function (e) {
        ...
        if (confirm(settings.textConfirmDelete)){
            ...
            newd  = $(this).closest('li').data(); 
            //console.log(newd['comment_id']);
            removedata(newd['comment_id']);
    ...
    function TButtonGroup() {
        ...
        var $btnEdit = TButton({classCss: 'btn btn-primary btn-sm btnEdit mydata', text: settings.labelEdit});
        ...
        $divbtn.append($btnUp).append($btnDown).append($btnOut).append($btnEdit).append($btnRemv);
        return $divbtn;
    }
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var $hiddenspan = $('<span>').addClass('hidspan').append(v.comment_id).css('display', 'none');
            var $divbtn =  TButtonGroup();
            $div.append($i).append("&nbsp;").append($span).append($hiddenspan).append($divbtn);
            $li.append($div);
            if (isParent) {
